...
| **★ Secrets** | Passwords in descriptions/files, **GPP**, **LAPS**, **gMSA/dMSA**, **DPAPI** (including non-admin user Credential Manager extraction over WinRM), pre-created computer passwords (**NetExec + SID/group-aware LDAP fallback**) → auto-pivot on everything recovered |
...
| **★ ACL/delegation abuse** | `GenericAll`, `WriteDACL`, `ForceChangePassword`, `AddSelf`, `WriteOwner`, **WriteSPN**, constrained delegation — with `--abuse` it performs the chain: idempotent group add/reset, **WriteSPN→Kerberoast**, **Shadow Credentials**, **RBCD** (including writable-group → `AllowedToAct` bridges via an owned pre2k/gMSA computer), S4U-to-a usable Domain Admin (with a **Bronze Bit** `-force-forwardable` fallback for Protected-Users / not-delegatable targets, CVE-2020-17049), and **DCSync** |
